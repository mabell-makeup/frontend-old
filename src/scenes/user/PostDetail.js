...
const PostInfo = ({navigation}) => {
  const {post, products, masterData} = useSelector(({postDetail: {post, products}, app: {masterData}}) => ({post, products, masterData}))
...
export const PostDetail = ({navigation}) => {
  const {post, postUser, isLoading} = useSelector(({postDetail: {post, postUser, isLoading}}) => ({post, postUser, isLoading}))
